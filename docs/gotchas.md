...
- Edit/delete use REST-ish routes: `GET/POST /<entity>/<id>/edit`, `GET /<entity>/<id>/row`, `DELETE /<entity>/<id>`. Handlers guard ownership (`SELECT 1 ... AND user_id` → `abort(404)`). Guards run before the write `try`, and **write-path handlers catch `except psycopg2.Error`, not `Exception`** — an `abort()`'s HTTPException can structurally never be swallowed into a 200. FK-delete sites branch on `psycopg2.errors.ForeignKeyViolation`. Deliberately still-broad catches (commented in place), and this list is the complete set as of #309: digests' per-user batch isolation, **reminders' two batch loops** (`materialize_all_users` and the `_push_pass` spine — same per-user isolation as digests, and they were missing from this list rather than from the code), ask's `dispatch` tool-error isolation, ai.py's model seams, `db_cursor`'s rollback+reraise. ⚠️ An enumeration that is silently incomplete reads as permission for the next one — if you add a broad catch, add it here
...
- **ALL app DB access goes through `db_cursor(commit=False)`** (`db.py`) — commits on clean exit, rolls back + re-raises on error, always returns the connection. ⚠️ **Connections are POOLED since #401** (idle cap `POOL_IDLE_MAX` = 4 per process, overflow opens a fresh one, pid-keyed so none crosses a fork). `return_connection` **rolls back on every return, the clean path included**: a read-only block never ends its transaction, and before the pool, closing ended it. Without that rollback the next borrower inherits uncommitted writes, `FOR UPDATE` locks or an aborted transaction (`tests/test_db_pool.py` fails both ways). Nothing may `SET` session state on a `db_cursor` connection, because it outlives the block. A pooled connection that dies (a Postgres restart) fails its next borrower once, then gets discarded. `get_db_connection()` remains only inside db.py and the test harness (`tests/helpers.py`, deliberately). Write pattern: ownership guard in its own read `with`, then `try: with db_cursor(commit=True):` around only the writes
- **Rows are namedtuples:** `db_cursor()` sets `cursor_factory=NamedTupleCursor` — read `row.amount`; positional access still works. Consequences: every SELECT column needs a **unique, valid-identifier name** (alias expressions — two unaliased `COALESCE(...)` columns raise `duplicate field name` at fetch); hand-built error-path rows use `row._replace(...)` or the module-level namedtuples (`CategoryRow`, `TxnEditRow`, `HistoryRow`, `GoalEditRow`, `BudgetRow`); a typo'd attribute in Jinja renders as EMPTY STRING, not an error — content-asserting tests are the net. `tests/helpers.py` stays on plain tuples — do NOT put the factory there
- Budgets are the cockpit (`/budgets`): one monthly amount per category, `POST /budgets/set` (upsert) / `POST /budgets/clear`; no edit/delete-by-id routes. `compute_budget_suggestions()` (6-mo avg, whole-dollar) seeds the default; `compute_budget_vs_actual(user_id, year, month)` is month-based; both in budgets.py. ⚠️ It returns `(category_id, category, budget, actual, remaining)` and is grouped by `c.id`, NOT `c.name` (#315) — two categories may share a name, and grouping by name merged them into one row comparing ONE budget against BOTH categories' spending, which `insights.compute_month_facts()` then handed to the month read as a real overrun. **Read its rows by attribute**; the id sits at the front, so a positional read gets the wrong column. `insights.category_spending()` still groups by name on purpose — summing a shared label is the right answer for a "what did I spend on X" rollup
...
- **Due-runner locking is now LOAD-BEARING, not merely prudent:** the due-row SELECTs are `FOR UPDATE`. They already guarded two simultaneous page loads (gunicorn serves on 4 threads); since `0.2.0` the **scheduler thread races those page loads too**. Keep the lock if those queries are ever touched — the two race scenarios in `tests/features/schedule_materialization.feature` are the net (removing `FOR UPDATE` turns both red; the pytest twin was deleted in #395), and `test_transfer_schedules.py`'s concurrency test covers transfers
...
- **Amount validation:** every form amount goes through `helpers.parse_positive_amount()` — `float('nan')` passes a plain `<= 0` check and Postgres stores NaN in numeric, poisoning every SUM(). Never hand-roll `float(x); if x <= 0`. ⚠️ Since `sql/38` (#314) **the database refuses a NaN too**, in all nine numeric columns — the validator is no longer the only thing standing between a NaN and a stored row, which matters because it is younger than the columns it guards
