...
- `test_doc_claims.py` also gained a seam-name check in #309: every `_call_*` identifier mentioned anywhere under `app/` or `tests/` must resolve to a real seam, or be listed in `DELIBERATELY_ABSENT_SEAMS` (empty since #396 deleted the Goal Coach tombstone, the only test that named a removed seam). ⚠️ **The first cut failed on its own docstring**, which spelled out an example dangling name — the same self-match that once failed a correct `Dockerfile`. Describe a wrong name, never write one. Verified red by planting a comment naming a seam that does not exist
