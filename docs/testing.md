...
- `test_ai_collapse.py` — ⚠️ **ONE card left** (#232): the goal coach on `/goals` is the only consumer of the read-state collapse, and therefore the only thing keeping `initAiCollapse` alive. Same properties as before — CLOSED with `data-ai-key`/`data-generated`, empty state OPEN, the generate fragment's `data-generated` EQUALS the next load's (the RETURNING round-trip) — plus one test that Home's panel is deliberately NOT a `<details>`
