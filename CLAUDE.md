...
- **`.devcontainer/` ATTACHES to the `web` service** — it does not define a second image, so the editor runs against the exact container that serves the app. ⚠️ **No Docker inside it**: `docker compose`, the `verify` skill, and **Claude Code** all belong in a Mac terminal. Claude Code started inside the container cannot reach anything outside the repo — including the Obsidian vault, which would silently break the end-of-session notes pass. The dev container is optional; a normal editor window falls back to `.vscode/settings.json` → `.venv`
- **The `dev` stage carries editor tooling too** — `git`, `procps` (`ps`), `curl` — because the base image is `python:3.11-slim` and has none of them. Without `git` the attached editor reports "git not installed" and its source-control view is dead. Prod has none of this (that is why the compose healthcheck probes through Python rather than curl)
- **⚠️ NEVER set `python.defaultInterpreterPath` in `.vscode/settings.json`** (#78). VS Code resolves **workspace** settings ABOVE **remote** ones, so a value there overrides `.devcontainer/devcontainer.json` instead of deferring to it — `${workspaceFolder}/.venv/bin/python` then follows you into the container, where `${workspaceFolder}` is `/app` and `/app/.venv` is a deliberately-masked EMPTY dir, producing "interpreter does not exist" and no import resolution. With the setting absent both cases work: the Python extension auto-discovers `.venv/` locally, and the devcontainer's own value applies in the container. The comment holding that spot in `settings.json` exists so it does not get helpfully re-added
- **`.venv/` is EDITOR-ONLY** — nothing is ever run from it; the app and tests stay in containers. It must be **Python 3.11** (Homebrew): the Mac's system 3.9 cannot evaluate `app/ai.py`'s `str | None` annotations and reports working code as broken. Gitignored (both `venv/` and `.venv/`) and in `.dockerignore`
